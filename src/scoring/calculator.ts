...
export function calculateScore(violations: Violation[], fileCount: number): number {
...
export function getScoreLabel(score: number): string {
  if (score >= 90) return "Immaculate Vibes";
  if (score >= 70) return "Good Vibes";
  if (score >= 50) return "Mid Vibes";
  if (score >= 30) return "Bad Vibes";
